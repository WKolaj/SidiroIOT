...
const {
  joiSchema,
} = require("../../../models/Elements/Alerts/BandwithLimitAlert");
const { exists } = require("../../../utilities/utilities");

const defaultBandwithLimitTranslationObject = {
  highLimit: {
...
class BandwidthLimitAlert extends Alert {
  ...
  constructor(project, device) {
    ...
    this._highAlertTurnOffValue = null;
    this._lowAlertActive = null;
    this._highAlertActive = null;
  ...
  /**
   * @description is low alert active
   */
  get LowAlertActive() {
    return this._lowAlertActive;
  }

  /**
   * @description is high alert active
   */
  get HighAlertActive() {
    return this._highAlertActive;
  }
   ...
   */
  _deactivateHighAlert() {
    this._highAlertActive = false;
  }

  /**
   * @description Method for activating high alert
   */
  _activateHighAlert() {
    this._highAlertActive = true;
  }

  /**
   * @description Method for deactivating low alert
   */
  _deactivateLowAlert() {
    this._lowAlertActive = false;
  }

  /**
   * @description Method for activating low alert
   */
  _activateLowAlert() {
    this._lowAlertActive = true;
  }
  ...
  async init(payload) {
    ...
    this._highLimit = payload.highLimit;
    this._lowLimit = payload.lowLimit;
    this._texts = exists(payload.texts)
      ? payload.texts
      : defaultBandwithLimitTranslationObject;
    ...
    this._highAlertTurnOffValue =
      this.HighLimit - Math.abs((this.HighLimit * this.Hysteresis) / 100);
  ...
  async refresh(tickId) {
    ...
    if (this.HighAlertActive) {
      if (value <= this._highAlertTurnOffValue) {
  ...
  checkIfValueCanBeSet(value) {
    return "BandwidthLimitAlert value is readonly";
  }
...
module.exports = BandwidthLimitAlert;
