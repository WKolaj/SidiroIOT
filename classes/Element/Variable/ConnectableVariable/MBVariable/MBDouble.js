...
const {
  checkDouble,
} = require("../../../../../models/Elements/ElementsValues/Double");
const {
  joiSchema,
} = require("../../../../../models/Elements/Variable/MBVariable/MBDouble");

class MBDouble extends MBVariable {
  //#region ========= CONSTRUCTOR =========
  ...
  _convertValueToData(value) {
    ...
    let floatValue = new Float64Array(1);
    floatValue[0] = value;
    let bytes = new Int8Array(floatValue.buffer);
   ...
   */
  async init(payload) {
    if (payload.type !== "MBDouble")
      throw new Error("Invalid type in payload of MBDouble");
    if (payload.length !== 4)
      throw new Error("Invalid length in payload of MBDouble");
